...
import { Client, Vehicule, Piece, Service, LigneFacturePiece, LigneFactureService, LigneDevisMainOeuvre } from '@/types';
...
const TAUX_HORAIRE_PAR_DEFAUT = 95;
...
export default function DevisForm({ onSubmit, loading, defaultClientId }: Props) {
  const [clients, setClients] = useState<Client[]>([]);
  const [vehicules, setVehicules] = useState<Vehicule[]>([]);
  const [pieces, setPieces] = useState<Piece[]>([]);
  const [services, setServices] = useState<Service[]>([]);
  const [tauxHoraireDefaut, setTauxHoraireDefaut] = useState(TAUX_HORAIRE_PAR_DEFAUT);
  ...
  const [lignesMainOeuvre, setLignesMainOeuvre] = useState<LigneDevisMainOeuvre[]>([]);
  ...
  const loadBaseData = () =>
    Promise.all([api.get('/clients'), api.get('/pieces'), api.get('/services'), api.get('/parametres')])
      .then(([c, p, s, param]) => {
        ...
        setTauxHoraireDefaut(param.data.taux_horaire_defaut);
      })
      .catch(() => {});
  ...
  const addMainOeuvre = () => {
    setLignesMainOeuvre((prev) => [
      ...prev,
      { description: '', heures: 1, taux_horaire: tauxHoraireDefaut },
    ]);
  };

  const updateMainOeuvreLigne = (index: number, field: keyof LigneDevisMainOeuvre, value: string | number) => {
    setLignesMainOeuvre((prev) => {
      const updated = [...prev];
      updated[index] = { ...updated[index], [field]: value };
      return updated;
  ...
  const totalMainOeuvre = lignesMainOeuvre.reduce((sum, m) => sum + m.heures * m.taux_horaire, 0);
  const sousTotal = totalPieces + totalServices + totalMainOeuvre;
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-gray-700">Services (prix fixe)</label>
            <button
              type="button"
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="text-sm font-medium text-gray-700">Main d&apos;œuvre (taux horaire)</label>
            <button
              type="button"
          ...
          {lignesMainOeuvre.length > 0 && (
            <div className="space-y-2">
              {lignesMainOeuvre.map((ligne, i) => (
                <div key={i} className="flex items-center gap-2 bg-gray-50 rounded-lg px-3 py-2">
                  <input
                    type="text"
                    value={ligne.description}
                    onChange={(e) => updateMainOeuvreLigne(i, 'description', e.target.value)}
                    placeholder="Description (ex. diagnostic, remplacement freins...)"
                  ...
                  <input
                    ...
                    step={0.25}
                    value={ligne.heures}
                    onChange={(e) => updateMainOeuvreLigne(i, 'heures', Number(e.target.value))}
                    className="w-16 border border-gray-300 rounded px-1.5 py-1 text-xs text-center focus:outline-none focus:ring-1 focus:ring-blue-500"
                    title="Heures"
                  />
                  ...
                  <input
                    ...
                    onChange={(e) => updateMainOeuvreLigne(i, 'taux_horaire', Number(e.target.value))}
                  ...
                  />
                  <span className="text-xs font-medium text-gray-700 w-16 text-right">
                    {(ligne.heures * ligne.taux_horaire).toFixed(2)} $
                  </span>
                  <button
                    type="button"
                    onClick={() => setLignesMainOeuvre((prev) => prev.filter((_, idx) => idx !== i))}
                    className="p-1 text-red-400 hover:text-red-600"
                  >
                    <Trash2 size={13} />
                  </button>
        ...
        <div className="border-t border-gray-100 pt-4 space-y-1 text-sm">
          ...
          <div className="flex justify-between text-gray-500">
            <span>Main d&apos;œuvre</span><span>{totalMainOeuvre.toFixed(2)} $</span>
          </div>
          <div className="flex justify-between text-gray-500">
