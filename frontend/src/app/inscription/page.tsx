...
const schema = z.object({
  nomUtilisateur: z.string().min(1, 'Nom requis'),
  nomGarage: z.string().min(1, 'Nom du garage requis'),
...
export default function InscriptionPage() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    setLoading(true);
    try {
      await signup(data.nomUtilisateur, data.nomGarage, data.email, data.password);
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center">
      <div className="bg-white rounded-xl shadow-md p-8 w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 mt-1 text-sm">Crée le compte de ton garage</p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div>
            ...
            {errors.nomUtilisateur && (
              <p className="text-red-500 text-xs mt-1">{errors.nomUtilisateur.message}</p>
            )}
          </div>
          ...
          <div>
            ...
            />
            {errors.nomGarage && (
              <p className="text-red-500 text-xs mt-1">{errors.nomGarage.message}</p>
            )}
          </div>
